...
def train(param, x, y):
  ...
  prior = distributions.MultivariateNormal(torch.zeros(dim_in).to(device), torch.eye(dim_in).to(device))
  ...
  # produce samples
  x_gen = flow.g(flow.prior.sample((100,))).detach().cpu().numpy()
  return x_gen
...
if __name__ == '__main__':
  ...
  print(scaler.as_dataframe(x_gen))
